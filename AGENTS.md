...
iOS Quick Add in `MacrosView.swift` queues items locally until the user saves the draft meal with `saveMealEntries(...)`; queued items appear in a floating tray pinned to the top of the add sheet over the Quick Add menu. Quick Add row buttons should remain add affordances: after a tap, show the green check/Added feedback only briefly, then return to the plus button so adding another of the same item still looks available.
