...
Onboarding tutorial spotlight geometry lives in `OnboardingView.swift`; clamp reported safe-area insets before positioning highlights because embedded `NavigationStack` backgrounds can report toolbar-inflated values. For top-toolbar targets, use an arrow pointer instead of a precision ring because toolbar measurement can still drift slightly across TestFlight/device chrome states.
