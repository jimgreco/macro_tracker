...
async function exportUserData(userId) {
  const [user, identities, entries, savedItems, foodCorrections, macroTargets, weightEntries, workoutEntries, sexualActivityEntries, sleepEntries, weightTarget, weightTargets, analysisReports, usageCounts, coachDismissals, clientDiagnostics, ouraConnection, ouraDocuments] =
    await Promise.all([
      ...
      pool.query(
        `SELECT oura_user_id, scopes, status, last_synced_at, last_webhook_at, last_error, created_at, updated_at
         FROM oura_connections WHERE user_id = $1`,
        [userId]
      ),
      pool.query(
        `SELECT data_type, provider_document_id, day, recorded_at, normalized_data, synced_at, created_at, updated_at
         FROM oura_documents
         WHERE user_id = $1 AND deleted_at IS NULL
         ORDER BY day DESC NULLS LAST, recorded_at DESC NULLS LAST`,
        [userId]
      )
    ]);
  ...
  return {
    ...
    oura: {
      connection: ouraConnection.rows[0] || null,
      documents: ouraDocuments.rows
    }
  };
}
