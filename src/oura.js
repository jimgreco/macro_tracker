...
function verifyOuraWebhookSignature({ timestamp, rawBody, signature, clientSecret }) {
...
function createOuraService({ db, env = process.env, fetchImpl = globalThis.fetch, logger = () => {} }) {
  ...
  async function fetchDocumentCollection(userId, dataType, startDate, endDate) {
    ...
    do {
      ...
      for (const document of documents) {
        ...
        // Collection reads represent current provider truth, so they can repair
        // a tombstone written after a transiently missing or out-of-order event.
        await db.upsertOuraDocument(userId, dataType, normalized, { resurrect: true });
      }
  ...
  async function fetchSingleDocument(userId, dataType, objectId) {
    const path = DATA_TYPE_PATHS[dataType];
  ...
  async function processWebhook(payload) {
    ...
    const dataType = String(payload?.data_type || '');
    ...
    const connection = await db.getOuraConnectionByProviderUserId(ouraUserId);
    ...
    try {
      ...
      // a durable tombstone for any event type.
      const result = await fetchSingleDocument(connection.userId, dataType, objectId);
  ...
  async function getStatus(userId) {
    ...
    return {
      ...
      expectedUpdateDelaySeconds: webhooksActive ? 60 : null,
  ...
  function startBackgroundJobs() {
    ...
    const interval = setInterval(run, reconciliationMinutes * 60 * 1000);
    initialTimer.unref?.();
    interval.unref?.();
    return () => {
      stopped = true;
