...
test('iOS Coach Tony P. exposes settings and title-first cards', () => {
  ...
  assert.ok(coach.includes('Text("\\(CoachBrand.name) - \\(suggestion.modelSource.label)")'));
  ...
  assert.ok(coach.indexOf('Text(suggestion.title)') < coach.indexOf('Text("\\(CoachBrand.name) - \\(suggestion.modelSource.label)")'));
...
test('iOS app includes onboarding reminders offline queue and diagnostics foundations', () => {
  ...
  assert.ok(onboarding.includes('tutorialPageBackground'));
  assert.ok(onboarding.includes('MacrosView()'));
  ...
  assert.ok(onboarding.includes('sync Health sleep'));
